$(function() {
    _("country").value = elem_obj["country"];
});
var master = {
    em1_lbl: "Home",
    em2_lbl: "Work",
...
function avatar_form_submitted() {
    $("#avatar_form").css("display", "block");
    $("#upfile_text").html('Uploading...');
}
        ...
        if (_(prop) != null && prop.slice(-3) != "lbl") {
            if (_(prop+"_container") != null && _(prop+"_container").style.display != 'none') {
                ...
                if (prop == "firstname" || prop == "lastname" || prop == "country") {
                    ...
                    var propval1 = _(prop+"_p1").value;
                    var propval2 = _(prop+"_p2").value;
                    var propval3 = _(prop+"_p3").value;
                    var propval4 = _(prop+"_p4").value;
                    var propval5 = _(prop+"_p5").value;
                    if (propval1 == "" || propval2 == "") {
                        ...
                        var adrval = ";;"+propval1+";"+propval2+";"+propval3+";"+propval4+";"+propval5;
                        list += prop+"="+adrval+"&";
                    }
                } else {
                    if (prop == "city") {
    ...
    ajax.onreadystatechange = function() {
        if(ajaxReturn(ajax) == true) {
            if (ajax.responseText == "info_change_success") {
                changestatus1.style.display = "block";
                changestatus1.innerHTML = "All changes saved";
                changestatus2.style.display = "block";
                changestatus2.innerHTML = "All changes saved";
                    ...
                    if (prop != "id" && prop != "username" && prop != "email" && prop != "email_lbl" && prop != "password" && prop != "gender" && prop != "userlevel" && prop != "avatar" && prop != "ip" && prop != "signup" && prop != "lastlogin" && prop != "notescheck" && prop != "activated") {
                        ...
                        if (_(prop) != null && prop.slice(-3) != "lbl") {
                            if (_(prop+"_container") != null && _(prop).value != elem_obj[prop]) {
                                if (prop.slice(0, 2) == "ad") {
                                    if (elem_obj[prop] != "") {
                                        var pieces = elem_obj[prop].split(';');
                                    } else {
                                        var str = ";;;;;;;;";
                                        var pieces = str.split(';');
                                    }
                                    _(prop+"_p1").value = pieces[2];
                                    _(prop+"_p2").value = pieces[3];
                                    _(prop+"_p3").value = pieces[4];
                                    _(prop+"_p4").value = pieces[5];
                                    _(prop+"_p5").value = pieces[6];
                                } else {
...
function clear_field(elem){
    ...
    if (input_id.slice(0, 2) == "ad") {
        _(input_id+"_p1").value = "";
        _(input_id+"_p2").value = "";
        _(input_id+"_p3").value = "";
        _(input_id+"_p4").value = "";
        _(input_id+"_p5").value = "";
    } else {
        _(input_id).value = "";
    }
